MPF = function(fit1, fit2, covarx, RE1, RE2) 
  {
      ...
      for (l in k:length(pot)) 
        {
         ...
         mfp2 = try(update(fit2, .~. + x1 + x2, 
                           random=RE2$reStruct, correlation=RE2$corStruct,
                           weights=RE2$varStruct), 
                    silent = FALSE)
                     ...
                     for (k in 1:length(pot)) 
                      {
                       ...
                       mfp1 = try(update(fit2, .~. + x1, 
                                         random=RE1$reStruct, correlation=RE1$corStruct,
                                         weights=RE1$varStruct), silent = FALSE)
